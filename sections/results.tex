...
% refactor(discussion) With Unity having a strong presence on both Steam and itch.io, it seems natural that developers would have been in contact with the game engine in advance.
% refactor(discussion) It is difficult to make a correct statement about other third party engines due to the lack of specificity.
% refactor(discussion) However, a statement, or rather an assumption, can be made about GameMaker.
% refactor(discussion) If you take a closer look at \autoref{table:gmtk}, it becomes clear that GameMaker is the only game engine that has lost an extremely large percentage of its users.
% refactor(discussion) It could therefore be possible that these developers have switched from GameMaker to the Godot Engine.\\
